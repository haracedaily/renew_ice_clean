...
function searchPlaces(keyword) {
    ...
    function createMarkers() {
        ...
        franchiseResults.forEach((place) => {
            ...
            kakao.maps.event.addListener(marker, 'click', function() {
                ...
                if (placeId) {
                    ...
                    // fallback: 기본 infoWindow
                    new kakao.maps.InfoWindow({
                        position: marker.getPosition(),
                        content: `<div style='padding:8px;min-width:200px;'><b>${place.place_name}</b><br>${place.road_address_name || place.address_name}<br>${place.phone || ''}</div>`
                    }).open(map, marker);
                }
            });
        ...
        if (franchiseResults.length > 0) {
            ...
            console.log('100m 반경 내에 유명 프랜차이즈 카페/커피점이 없습니다.');
        }
    }
